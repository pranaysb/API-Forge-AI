...
print("Running graph normally and inspecting get_state().next at each yield...")
stream_generator = graph.stream(initial_state, config)
for s in stream_generator:
    node_name = list(s.keys())[0]
    state = graph.get_state(config)
    print(f"Yielded from: {node_name} | State.next is now: {state.next}")
    if node_name == "coder":
        break

print("\nInterrupting. Resuming with stream(None, config)...")
for s in graph.stream(None, config):
    node_name = list(s.keys())[0]
    state = graph.get_state(config)
    print(f"RESUME Yielded from: {node_name} | State.next is now: {state.next}")
